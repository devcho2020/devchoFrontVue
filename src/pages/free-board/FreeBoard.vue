<script setup>
  ...
  const freeBoardTotalPage = ref(1);
  ...
  const fnMoveFreeBoardDetail = (freeBoardId) => {
    router.push({
      path: `/free-board/${freeBoardId}`,
      query: route.query
    })
  };

  const fnSearchFreeBoardList = (p) => {
  ...
  const fnGetFreeBoardList = async ({p = 1, opt = 'all', s = ''} = {}) => {
    if (isLoading.value) return;
    try {
      isLoading.value = true;

      const response = await api.get('/free-board', {
        params: {
          page: p,
          size: 10,
          selectedOption: opt,
      ...
      freeBoards.value = response.data.content;
      freeBoardTotalPage.value = response.data.totalPages || 1
    } catch (e) {
      console.error(e);
    } finally {
  ...
  <div class="border-slate-800 space-y-8">
    <div class="flex flex-col justify-between items-center gap-4">
      <div class="flex w-full justify-between">
        <div class="flex w-2/3 gap-2">
          ...
          <CommonButton
              variant="gray"
              @click="fnSearchFreeBoardList"
          >검색</CommonButton>
        </div>
        <CommonButton
            @click="fnMoveFreeBoardWrite"
        >글작성</CommonButton>
      </div>
    </div>
    <div class="bg-slate-900/30 rounded-xl border border-slate-800 overflow-hidden">
      <table class="table-fixed w-full text-left border-collapse">
        ...
        <tr v-if="!isLoading && freeBoards.length === 0">
        ...
        <tr v-if="!isLoading &&  freeBoards.length > 0" v-for="freeBoard in freeBoards" :key="freeBoard.id"
            @click="fnMoveFreeBoardDetail(freeBoard.id)"
        ...
        >
          ...
          <td class="px-6 py-4 text-xs text-slate-500 font-mono text-center">
            {{ freeBoard.creator?.userName }}
          </td>
          <td class="px-6 py-4 text-xs text-slate-500 font-mono text-center">
            <CommonDateFormat
                :date="String(freeBoard.createdAt)"
            />
          </td>
        </tr>
        </tbody>
